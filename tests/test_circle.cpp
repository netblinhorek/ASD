...
TEST(TestCircle, test_circles_externally_touch) {
    ...
    // Act & Assert
    ASSERT_EQ(std::string("External touch"), get_circles_relation(c1, c2));  //Внешнее прикосновение
}
TEST(TestCircle, test_circles_internally_touch) {
    Circle c1(0, 0, 5), c2(3, 0, 2);

    // Act & Assert
    ASSERT_EQ(std::string("Inner touch"), get_circles_relation(c1, c2)); //Внутреннее прикосновение
}
TEST(TestCircle, test_circles_intersect) {
    Circle c1(0, 0, 5), c2(6, 0, 5);

    // Act & Assert
    ASSERT_EQ(std::string("Intersect"), get_circles_relation(c1, c2)); //Пересекаться
}
...
TEST(TestCircle, test_circles_separate) {
    ...
    // Act & Assert
    ASSERT_EQ(std::string("Not intersected"), get_circles_relation(c1, c2)); //Не пересекались
}

TEST(TestCircle, test_almost_coincide) {
    Circle c1(0, 0, 1000), c2(0.0001f, 0.0001f, 1000);

    ASSERT_EQ(get_circles_relation(c1, c2), "Intersect");
}
